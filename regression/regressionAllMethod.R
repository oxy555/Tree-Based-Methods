...
# ------ 基于交叉验证 ------
bagging = function(training, test, B = 100, tolS = 1, tolN = 4){
  # 其实m = m1；也可不算test的列数
  ...
  for (i in 1:B) {
    # 利用boostrap从训练集中有放回地抽样B次，
    # 每次抽取的样本大小等于训练集的大小，在每个训练集上分别拟合树模型
    index = sample(n, n, replace = T)
    trainBstrap = training[index, ]
    tree = createTree(trainBstrap, tolS, tolN)
    # 根据拟合出来的树模型对测试集进行预测
...
# ----- 基于 out of bag -----
baggingOOB = function(dataSet, B = 100, tolS = 1, tolN = 4) {
  n = nrow(dataSet)
  m = ncol(dataSet)
  yHat = rep(0, n)
  ncount = rep(0, n)
  ...
  for (i in 1:B) {
    ...
    tree = createTree(train, tolS, tolN)
...
# ------ 基于交叉验证 -----
randomForestChen = function(training, test, mtry = 6, B = 400, tolS = 1, tolN = 4){
...
# ----- 基于out of bag -----
randomForestOOBChen= function(dataSet, mtry = 6, B = 400, tolS = 1, tolN = 4) {
  n = nrow(dataSet); 
  m = ncol(dataSet)
  p = m-1
  yHat = rep(0, n)
  ...
  for (i in 1:B) {
    ...
    index_var = sample(p, mtry)
    
    # 生成树的训练集是数据集含y的, 预测的函数是去掉因变量的数据集
...
gbdt = function(dataSet,threshold = 300, tolS = 1, tolN = 4) {
  ...
  while(RSS_train > threshold) {
    myTree = createTree(data1, tolS, tolN) # 在data1上训练树
