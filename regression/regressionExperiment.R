...
myTree = createTree(BostonTrain, tolS = 0, tolN = 1.1)
yHat = createForeCast(myTree, as.matrix(BostonTest[,-ncol(BostonTest)]))
mse_singe_chen = mean((yHat - BostonTest[,ncol(BostonTest)])^2) 


# ---- bagging.chen -----
mse_bagging_chen = bagging(Boston[train, ], Boston[-train, ], tolS = 0, tolN = 1.1)


# ---- out of bag.chen ----
mse_OOB_chen = baggingOOB(Boston, tolS = 0, tolN = 1.1);
mse_OOB_chen


# ---- randomForest.chen ----
mse_rf_chen = randomForestChen(Boston[train, ], Boston[-train, ], tolS = 0, tolN = 1.1)


# ---- randomForest_out of bag.chen ----
mse_rf_OOB_chen = randomForestOOBChen(Boston, tolS = 0, tolN = 1.1)
mse_rf_OOB_chen

# ---- gradeint boosting desicion tree ----
mse_gbdt_chen = gbdt(Boston, threshold = 250, tolS = 0, tolN = 1.1)
